...
{   
    ...
    tie(ei,end) = out_edges(v.first, rlc.g.g);
    
    cout << "Street edges : ";
    for(; ei != end; ei++) {
        Edge e = rlc.g.g[*ei];
        cout << "("<< source(*ei, rlc.g.g) <<", "<< target(*ei, rlc.g.g) <<", "<<e.type<<", "<<e.duration(0,0)<<")   ";
    }
...
source(source), dest(dest), start_sec(start_sec), start_day(start_day), 
heap(Compare(&(this->arr_times)))
{
    this->graph = graph;
    
    const int g_num_vert = num_vertices(graph->g.g);
...
std::list< int > Dijkstra::run()
{
    ...
    set_gray(rlc_start);
    {
        ...
        tie(g_tei,tend) = edges(graph->g.g);
    ...
    if(black(rlc_dest)) 
    {
        for(curr = rlc_dest; curr != rlc_start; curr = graph->source(get_pred(curr))) 
            path.push_front( graph->g.g[get_pred(curr).first].edge_index );
    } 
